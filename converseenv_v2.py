...
"""
# region import
from helpful_files.pentropy_main import *
import itertools
import json
...
# region EnvClass
class ConverseEnv():
    def __init__(self,N:int,K:int,RENDER="",point_num:int=12,generate_size:int=30,subset_size:int=30,comb_size:int=30):
        """
        主要用于各项参数的设置。
        """
        self.N = N
...
if __name__ == "__main__":
    ...
    for i in range(15):
        ...
        env.render(save_dir=f"E:/pycharm/python_doc/learntoconverse_reconstructed/01example/best_from_xxy_20250808/test_result/N={env.N},K={env.K}/low_speed_reason",namestring=f"episode={env.episode},time={time_now},total_time={total_time},total_reward = {round(sum(env.reward_list),5)}") # step 之后才可以进行render哎。
        if done:
            break
