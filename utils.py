...
class Config():
    def __init__(self, DBM = True, continue_training = False, envs_NK=[[2,2],[2,3]], use_pretrained_model=False, pretrained_model_id = None, new_ineq_num_factor = 0.5, num_worker = 1):
        ...
        self.about_training = """ok, real training begins, the best retrained model is 003."""
...
def draw_rewards_20points(log:dict, config:Config):
    ...
    for env_id in range(len(config.envs_NK)):
        ...
        # 绘制曲线和所有数据点
        plt.plot(env_rewards, alpha=0.3, color=color, label=str(config.envs_NK[env_id]))
        
        
        # 为每个环境曲线添加数值标注（仅在选定点上）
        for pos in annotation_indices:
            if 0 <= pos < len(env_rewards):
                plt.scatter(pos, env_rewards[pos], color=color, s=5, alpha=0.5, zorder=4)
