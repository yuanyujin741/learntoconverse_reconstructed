...
class Config():
    def __init__(self, DBM = True, continue_training = False, envs_NK=[[2,2],[2,3]], use_pretrained_model=False, pretrained_model_id = None,pretrained_model_checkpoint = "latest", new_ineq_num_factor = 0.5, num_worker = 1, rewardtype:Literal["innerbound_ratio", "original"] = "original", test_mode = False, test_task_id = "000", test_task_models = [], test_num = 3, test_subject = "Nothing"):
        ...
        self.about_training = """ test seed 01 """
    ...
    def set_policy_config(self, max_rollout_num = 10, n_directions = 4, evaluate_time = 3, learning_rate = 0.01, gamma = 0.97, std = 0.02, using_FiLM:bool=True):
        """
    ...
    def set_intime_config(self, episode_num:int):
...
def rollout(env:ConverseEnvWrapper, policy, rollout_length, gamma, NumFactor,config:Config):
    ...
    if config.test_mode == True:
        ...
        ob, _ = env.reset(seed = generate_seed(config, env))
...
def draw_rewards_20points(log:dict, config:Config):
    ...
    plt.xlabel('Evaluation rollout number')
    plt.ylabel('Rewards')
    plt.title('Training Rewards (20 Points Displayed)')
    plt.legend(loc="best")
    plt.grid(alpha=0.2)
    plt.tight_layout()
...
# region OTHER
# 主要是一些应该放在前面但是没有办法放在前面的
def generate_seed(config:Config,env:ConverseEnvWrapper):
    """
    生成seed若干，主要用于训练时保持环境的随机性；同时算法generate_seed是确定的保证了训练的可复现性。
    """
    seed_num_each = config.evaluate_time + 1 # 测试给不同的初始环境用于评估；计算给相同的初始环境用于epsilon效果比较
    offset = max(0, env.epsilon_id - config.n_directions + 1)
    seed = 0 + config.current_episode_num * seed_num_each + offset
    return int(seed%(1e6))
...
if __name__ == "__main__":
    ...
    # test TWO
    """
    seed:先只考虑train时候的seed的问题。而不是一起考虑test。
    1. set_intime_config √
    2. set_seed √
    3. generate_seed （也是只处理了train的seed，参见reset）
    """
    config = Config()
    config.set_attention_network_config(device="cuda")
    config.set_policy_config()
    config.set_intime_config(episode_num=1)
    set_seed(config)
    envs = make_converse_env(config)
    for i in range(len(envs)):
        print(envs[i][0].N,envs[i][0].K,generate_seed(config,envs[i][0]))
    config.set_intime_config(episode_num=2)
    for i in range(len(envs)):
        print(envs[i][0].N,envs[i][0].K,generate_seed(config,envs[i][0]))
